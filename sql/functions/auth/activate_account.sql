...
CREATE OR REPLACE FUNCTION auth.activate_account( activation_string char(64)) RETURNS INTEGER AS $$
  DECLARE
    ...
    p_activation_string ALIAS FOR $1;
  BEGIN
    ...
    SELECT INTO cur_activation account_id, conference_id FROM auth.account_activation WHERE account_activation.activation_string = p_activation_string;
    IF FOUND THEN
      ...
      DELETE FROM auth.account_activation WHERE account_activation.activation_string = p_activation_string;
    ELSE
      PERFORM 1 FROM log.account_activation WHERE account_activation.activation_string = p_activation_string AND account_activation.log_operation = 'D';
      IF FOUND THEN
