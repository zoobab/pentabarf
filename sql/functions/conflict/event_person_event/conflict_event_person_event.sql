  ...
  BEGIN
    ...
    LOOP
      FOR cur_conflict_event_person_event IN
        EXECUTE 'SELECT '|| quote_literal(cur_conflict.conflict) ||' AS conflict, event_id1, event_id2, person_id FROM conflict.conflict_' || cur_conflict.conflict || '(' || v_conference_id || ');'
      LOOP
